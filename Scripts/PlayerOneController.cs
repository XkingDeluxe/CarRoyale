...
public class PlayerOneController : MonoBehaviour
{
    ...
    private void Awake()
    {
        //Setup Inputs
        inputs = new Inputs();
        inputs.PlayerOne.Gas.performed += ctx => gas = ctx.ReadValue<float>();
        inputs.PlayerOne.Back.performed += ctx => back = ctx.ReadValue<float>();
        inputs.PlayerOne.Gas.canceled += ctx => gas = 0;
        inputs.PlayerOne.Back.canceled += ctx => back = 0;

        inputs.PlayerOne.Left.performed += ctx => right = ctx.ReadValue<float>();
        inputs.PlayerOne.Right.performed += ctx => left = ctx.ReadValue<float>();
        inputs.PlayerOne.Left.canceled += ctx => right = 0;
        inputs.PlayerOne.Right.canceled += ctx => left = 0;

        inputs.PlayerOne.cLeft.performed += ctx => cleft = ctx.ReadValue<float>();
        inputs.PlayerOne.cRight.performed += ctx => cright = ctx.ReadValue<float>();
        inputs.PlayerOne.cUp.performed += ctx => cup = ctx.ReadValue<float>();
        inputs.PlayerOne.cDown.performed += ctx => cdown = ctx.ReadValue<float>();
        inputs.PlayerOne.cBack.performed += ctx => cback = 5f;
        inputs.PlayerOne.cLeft.canceled += ctx => cleft = 0;
        inputs.PlayerOne.cRight.canceled += ctx => cright = 0;
        inputs.PlayerOne.cUp.canceled += ctx => cup = 0;
        inputs.PlayerOne.cDown.canceled += ctx => cdown = 0;
        inputs.PlayerOne.cBack.canceled += ctx => cback = 0;

        inputs.PlayerOne.Reset.performed += ctx => resetToPoint();
        inputs.PlayerOne.Boost.performed += ctx => applyBoost();
        inputs.PlayerOne.Boost.canceled += ctx => stopBoost();
    ...
    private void FixedUpdate()
    {
        ...
        if (start)
        {
            ...
            if (!reset)
            {
                ...
                //Set camera offset
                Camera1.off2 = new Vector3((cright - cleft), (cup - cdown), cback * 2);
            }
            else
            {
                //Reset motor speed
    ...
    private void OnTriggerEnter(Collider c)
    {
        ...
        if (c.name.Equals("finish"))
        {
            ...
            {
                start = false;
                PlayerTwoController.start = false;
                PlayerPrefs.SetInt("P1WINS", PlayerPrefs.GetInt("P1WINS") + 1);
                new Thread(finishCounter).Start();
            }
        }
    }
